...
**World & layout** (`src/world/`): the map is 96×72 tiles of 16px. `world_layout.gd` is the source of truth for blocked rects, tree/bench/flower cells, and the 40 hand-placed `TREASURE_CELLS`. `world.gd` builds TileMapLayers, props, and collisions procedurally in `_ready()` and draws building outlines in `_draw()`. Spawn selection (`treasure_spawner.gd.choose_spawn_index`) filters candidate points by the 18 m minimum distance and "not the previous point", falling back to the farthest valid point.
